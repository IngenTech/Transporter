...
public class DateTimePickerDialog extends Dialog {
    ...
    public interface DateTimeListener {
        void onDateTimeSelected(int year, int month, int day, int hour, int min, int sec, int am_pm);
    }
    ...
    public DateTimePickerDialog(Context context, final boolean is24HrView, DateTimeListener listener) {
        ...
        ((Button) findViewById(R.id.SetDateTime)).setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                int sec = mDateTimePicker.get(Calendar.SECOND);
                int aa = is24HrView ? -1 : mDateTimePicker.get(Calendar.AM_PM);
                System.out.println("DateTimePickerDialoug : "+year+"/"+month+"/"+day+" "+hour+":"+min+":"+sec);
                dateTimeListener.onDateTimeSelected(year, month, day, hour, min,sec, aa);
                dismiss();
            }
        });
